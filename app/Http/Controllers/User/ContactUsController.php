...
class ContactUsController extends Controller
{
    ...
    // {
    //     $this->sanitizeRequest(); // 🚀 Auto sanitize semua input (except allowHtml field kita handle manual)
    // }
    ...
    {
        try {
            // 💡 Rate limit per IP: max 5 request / 60 detik
            ...
            // 🧼 Bersihkan & allow safe HTML di message_contact
            $sanitized = [
                'name'            => $request->name, // Sudah auto sanitize via Trait
                'email'           => $request->email,
                'phone_number'    => $request->phone_number,
                'type_service'    => $request->type_service,
                'message_contact' => SanitizeHelper::sanitize($request->input('message_contact'), true), // Allow HTML aman
            ];
            ...
            $contact_us = ContactUs::create([
                ...
                'message'     => $sanitized['message_contact'], // Sudah safe HTML
            ]);
