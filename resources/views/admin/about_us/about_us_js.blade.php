<script src="https://cdn.jsdelivr.net/npm/bootstrap-fileinput@5.5.4/js/fileinput.min.js"
    integrity="sha256-eyql5FyEi9+B/fPWI8q0rZRgbYtBuzXmkO0XAqMq+Bg=" crossorigin="anonymous"></script>
    ...
    $(document).ready(() => {
        ...
        function fetchAllContentAboutUs() {
            $.ajax({
                ...
                success: function(res) {
                    ...
                    header != null ? $('#btn-header-about-us').attr('data-header_id', header.id) :
                        $('#btn-header-about-us').attr('data-header_id', '')
                    let data_header_id = $('#btn-header-about-us').data('header_id')

                    history != null ? $('#btn-history-about-us').attr('data-history_id', history
                        .id) : $('#btn-history-about-us').attr('data-history_id', '')
                    let data_history_id = $('#btn-history-about-us').data('history_id')

                    why_pralon != null ? $('#btn-why-pralon-about-us').attr('data-why_pralon_id',
                        why_pralon.id) : $('#btn-why-pralon-about-us').attr(
                        'data-why_pralon_id', '')
                    let why_pralon_id = $('#btn-why-pralon-about-us').data('why_pralon_id')

                    vision != null ? $('#btn-visi-misi-about-us').attr('data-vision_id', vision
                        .id) : $('#btn-visi-misi-about-us').attr('data-vision_id', '')
                    let data_vision_id = $('#btn-visi-misi-about-us').data('vision_id')

                    value_pralon != null ? $('#btn-value-with-contact-us-about-us').attr(
                        'data-value_pralon_id', value_pralon.id) : $(
                        '#btn-value-with-contact-us-about-us').attr('data-value_pralon_id', '')
                    let data_value_pralon_id = $('#btn-value-with-contact-us-about-us').data(
                        'value_pralon_id')

                    certificates != null ? $('#btn-sertifikat-about-us').attr('data-certificate_id',
                        certificates.id) : $('#btn-sertifikat-about-us').attr(
                        'data-certificate_id', '')
                    let data_certificate_id = $('#btn-sertifikat-about-us').data(
                        'data-certificate_id')
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (why_pralon_data == null) {
                        ...
                        $('#link_why_pralon_image').empty()
                        $('#link_why_pralon_image').append(
                            `<a href="" id="link_source_why_pralon_image" target="_blank">Link Image</a>`
                            )
                        ...
                        $.each(detail_why, (i, detail) => {
                            // alert(i)
                            let class_array = i != 0 ?
                                'array_list-detail-why-pralon' : '';
                            let class_button = i != 0 ?
                                'btn btn-danger pop_dynamic_detail_why' :
                                'btn btn-info add_dynamic_detail_why';
                            let class_icon = i != 0 ? 'fas fa-minus' :
                            'fas fa-plus';
                            $('.list-detail-why-pralon').append(`
        ...
        function updateIdWhyPralon() {
            $('.array_list-detail-why-pralon').each(function(i) {
                ...
                $(this).find('input[name="detail_why_pralon_title[]"]').attr('id',
                    detail_why_pralon_title);
                $(this).find('input[name="detail_why_pralon_icon[]"]').attr('id',
                    detail_why_pralon_icon);
                $(this).find('input[name="detail_why_pralon_desc[]"]').attr('id',
                    detail_why_pralon_desc);
            })
        }
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (visi_misi_data == null) {
                        ...
                        $('#link_visi_misi_image').empty()
                        $('#link_visi_misi_image').append(
                            `<a href="" id="link_source_visi_misi_image" target="_blank">Link Image</a>`
                            )
                        ...
                        $.each(misions, (i, misi) => {
                            ...
                            let class_button = i != 0 ?
                                'btn btn-danger pop_dynamic_form_misi' :
                                'btn btn-info add_dynamic_form_misi';
                            let class_icon = i != 0 ? 'fas fa-minus' :
                            'fas fa-plus';
                            let numbering = i + 1
                            $('.list-misi').append(`
                ...
                success: function(res) {
                    ...
                    if (value_data == null) {
                        ...
                        $.each(value_data, (i, value) => {
                            ...
                            let class_button = i != 0 ?
                                'btn btn-danger pop_dynamic_form_value' :
                                'btn btn-info add_dynamic_form_value';
                            let class_icon = i != 0 ? 'fas fa-minus' :
                            'fas fa-plus';
                            let numbering = i + 1
                            $('.list-value').append(`
